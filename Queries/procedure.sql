...
BEGIN TRANSACTION T1
	IF EXISTS (SELECT * FROM Compensation.tblEstimatePrice WHERE [Land Owner ID] = @LandOwnerId AND [Project ID] = @projectID) 
		BEGIN
			UPDATE  Compensation.tblEstimatePrice SET [Amount] =Compensation.fnTotalComp( @landOwnerID)  WHERE [Land Owner ID] = @LandOwnerId 
...
GO
GO
--CREATE  STORE PROCEDURE TO CHEDCK PAYMENT CHECK
